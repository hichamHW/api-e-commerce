...
class SecurityTokensService {
    ...
    const customer = await db.collection('customers').findOne({email, hash: {$exists: true}});
    if (customer) {
      ...
      if(match) {
          var token = {
            ...
            expiration:12,
            scopes:[
              security.scope.READ_CUSTOMERS,
              security.scope.WRITE_CUSTOMERS,
          ...
          return  this.getSignedToken(token)
                  .then(signedToken => {
                    return {customerId:customer._id,token:signedToken}
                  })
                  .catch((error)=>{
                    return error
                  })
      }else{
        return Promise.reject({message: 'The password is incorrect'})
      }
    }else{
      return Promise.reject({message: 'The account is not existed'})
    }
  }

   signInSocial(token,type) {
     return new Promise((resolve,reject)=>{
        this.getSocialUserInfo(token,type)
  	    .then(response => {
          if (response.id != undefined) {
            db.collection('customers').findOne({socialId: response.id, socialType: type},(error,item)=>{
              if (item) {
                var customerToken = {
                  id:item._id,
                ...
                this.getSignedToken(customerToken)
                .then(signedToken => {
                    resolve({customerId:item._id,token:signedToken})
                })
                .catch(reject)
              }else{
                let customer = {
                ...
                customer.full_name = response.name;
                customer.socialId = response.id;
                customer.socialType = type;

                db.collection('customers').insert(customer,(error,item)=>{
                  var customerToken = {
                    id:item._id,
                    email: response.email,
                    expiration:12,
  ...
  getSocialUserInfo(token,type){
    return new Promise((resolve,reject)=>{
      var url = null
      if (type == 'facebook') {
        url = "https://graph.facebook.com/me?fields=name,id,email,picture&access_token="+token
      }
