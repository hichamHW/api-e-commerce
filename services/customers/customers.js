...
class CustomersService {
    ...
    // is email unique
    if (customer.email && customer.email.length > 0) {
      const user = await db.collection('customers').findOne({email: customer.email, hash: {$exists: true}});
      if(user && user.hash != undefined){
        return Promise.reject({message: 'The email is existed'})
      }
    }
    const hash = await bcrypt.hash(data.password, saltRounds);
    customer.hash = hash
    const insertResponse = await db.collection('customers').insertMany([customer]);
    const newCustomerId = insertResponse.ops[0]._id.toString();
    const newCustomer = await this.getSingleCustomer(newCustomerId);
